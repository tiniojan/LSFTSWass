...
path_fig = '../figs'
process = 'GaussiantvFAR(1)'

ttest =True
device = torch.device("cpu")

# Define the construct_A_u function
def construct_A_u(J, u):
    i_indices = np.arange(1, J + 1, dtype=float).reshape(-1, 1)
    j_indices = np.arange(1, J + 1, dtype=float).reshape(1, -1)
    sigma_ij = np.sqrt(u * i_indices**(-6) + (1 - u) * np.exp(-np.abs(i_indices - j_indices)))
    A_u = np.random.normal(0, sigma_ij, size=(J, J))
    singular_values = np.linalg.svd(A_u, compute_uv=False)
    schatten_infinity_norm = np.max(singular_values)
    return A_u, schatten_infinity_norm

# Define the construct_B_t function
def construct_B_t(J, u):
    A_u, schatten_infinity_norm = construct_A_u(J, u)
    B_t = (0.4 * A_u) / schatten_infinity_norm if schatten_infinity_norm != 0 else 0.4 * A_u
    return B_t
...
def simulation_covariate(T, N, J, tau):
    ...
    for t in range(1, T):
        u = t / T
        B_t_matrix = construct_B_t(J, u) 
        eta_t = eta_t_fourier(J, tau)    
        curve[t, :] = B_t_matrix @ curve[t - 1, :] + eta_t
...
def plot_results(times_t_dict, times_T, n_replications, iterations, process, wass_distances_stats, time_kernel, space_kernel, input_dir):
    ...
    plt.tight_layout()
    plt.savefig(f"EW1_tvFAR(1)_timekernel-{time_kernel}_spacekernel-{space_kernel}_rep-{n_replications}_iter-{iterations}.pdf", dpi=300)
    plt.show()
...
def main():
    ...
    input_dir = "tvFAR1_Wass"
    output_dir = "tvFAR1WassStats"
    os.makedirs(output_dir, exist_ok=True)
