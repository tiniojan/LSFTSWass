...
def main():
    ...
    df_Nikkei225 = pd.read_csv(path_data + 'NIKKEI225.csv')
    df_Nikkei225.head(), df_Nikkei225.shape

    sns.set(style="darkgrid")
    plt.rcParams["text.usetex"] = False
    plt.rcParams["figure.figsize"] = (10,3)
    plt.plot(df_Nikkei225['NIKKEI225'], label="original data")
    plt.legend()
    plt.xlabel("$s$", fontsize= 16)
    plt.ylabel("Nikkei225", fontsize= 16)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.show()

    # Load data 
    time_series = df_Nikkei225['NIKKEI225'].values 


    # Reshape into continuous sample curves
    num_samples = 60 # To increase T, refine the splitting to 30 and 15
    ...
    plt.ylabel("Nikkei225", fontsize= 20)
    ...
    plt.xticks(np.linspace(1, T, 7, dtype=int))
    plt.show()
    ...
    plt.xticks(np.linspace(1, T, 7, dtype=int), fontsize=18)
    plt.yticks(np.linspace(min(Y.flatten()), max(Y.flatten()), 5), fontsize=18) 
    plt.legend()
    plt.show()

    # Simulation parameters setting
    times_t = [95, 102, 108, 115, 121, 128, 135, 140] #Change this when changing the splitting parameter
    j_selected = np.array([10, 30, 50], dtype=np.int64)  #Change this when changing the splitting parameter
    times_sigma = np.array([0.1, 0.01, 0.001, 0.0001], dtype=np.float64)
    n_replications = 500 # Increase to 500
